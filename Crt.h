...
#define RVA(Instr, InstrSize) ((__int64)Instr + InstrSize + *(LONG*)((__int64)Instr + (InstrSize - sizeof(LONG))))
...
public:
    ...
    pointer datar() noexcept {
        return data;
    }

    const_pointer datar() const noexcept {
        return data;
    }
...
namespace md5 {
	using Digest = ArrayR<unsigned char, 16>;
	...
	namespace details {
		...
		constexpr Digest make_digest(const ArrayR<uint32_t, 4>& input) noexcept {
			Digest digest{};
		...
		struct Context {
			ArrayR<unsigned char, CBLOCK> buffer;
			ArrayR<uint32_t, 4> state;
			uint32_t nl, nh;
			...
			void transform(const ArrayR<uint32_t, LBLOCK>& input) noexcept {
	...
	template <size_t N>
	Digest compute(const char(&data)[N]) noexcept {
		details::Context c;
	...
	Digest compute(const char* s) noexcept {
		details::Context c;
		// Don't hash the null-terminator
