...
#define xorstr_t(str) ::jm::xor_string<XORSTR_STR(str)>
...
// i7 2600k didn't support this owo
...
namespace jm {
	...
	namespace detail {
		...
		{
			static_assert(M <= 99, L"string too large.");
			return (I < M) ? str[I] : 0;
		}
		...
		constexpr uintptr_t key8()
		{
			constexpr auto first_part = key4<2166136261 + S>();
			constexpr auto second_part = key4<first_part>();
			return (static_cast<uintptr_t>(first_part) << 32) | second_part;
		}
		...
		struct string_storage {
			uintptr_t storage[buffer_size<T>()];
			...
			XORSTR_FORCEINLINE constexpr string_storage() : storage{ 0 }
			{
				...
				for (std::size_t i = 0; i < T::size; ++i)
					...
					(uintptr_t{ static_cast<cast_type>(T::str[i]) }
	...
	struct xor_string {
		alignas(detail::buffer_align<T>())
			XORSTR_VOLATILE uintptr_t _storage[detail::buffer_size<T>()];
		...
		XORSTR_FORCEINLINE void _crypt() noexcept
		{
			if constexpr (detail::buffer_size<T>() > N) {
				...
				if constexpr ((detail::buffer_size<T>() - N) >= 4) {
					...
					// = { ... }
					alignas(32) XORSTR_CLANG_VOLATILE uintptr_t keys[4];
...
#endif
				{
					alignas(16) XORSTR_VOLATILE uintptr_t keys[2];
					keys[0] = detail::key8<N + 0>();
					keys[1] = detail::key8<N + 1>();
		...
		template<std::size_t N>
		XORSTR_FORCEINLINE constexpr static uintptr_t _at()
		{
			// forces compile time evaluation of storage for access
			constexpr uintptr_t val = detail::string_storage<T>{}.storage[N];
			return val;
		}
